...
df2 <- read.table("G:/My Drive/01. BMKG/MOS/data/data_complete_rh.txt", sep=',', header=T)
...
ggplot(df, aes(x=Date, y=RH.obs, color=lokasi))+
  geom_line(na.rm=TRUE)+
  stat_smooth(colour="green")+
  facet_wrap(~lokasi)+
  labs(title = "Humidity timeseries plot",
...
df2 %>% 
  ggplot(., aes(x=YM, y = RH.obs))+geom_boxplot()
...
# write.table(df, 'G:/My Drive/01. BMKG/MOS/data/data_fix_rh_obs.txt', sep=',')
