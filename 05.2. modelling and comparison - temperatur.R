...
df <- read.table("G:/My Drive/01. BMKG/MOS/data/data_fix_T_obs.txt", sep=',')
ls()
str(df)
...
for (i in 1:length(loc)) {
    ...
    # CORRELATION
    # scatter plot
    featurePlot(x = df1[, !names(df1) %in% c("T.obs")], y = df1[, names(df1) %in% c("T.obs")],
    ...
    Rsquare.baseline = R2_Score(testData[,3], testData[,30])
    Rsquare.baseline
    mape.baseline <- MAPE(testData[,3], testData[,30])
    mape.baseline
    rmse.baseline <- RMSE(testData[,3], testData[,30])
    rmse.baseline
    mse.baseline <- MSE(testData[,3], testData[,30])
    mse.baseline
    ...
    ######################################################## linear regression 
    set.seed(669); ptm <- proc.time()
    linearReg <- train(T.obs ~  . , data = trainingData, method = "lm", trControl = controlObject) 
    ...
    ######################################################## random forest 
    # set.seed(669); ptm <- proc.time()
    # rfModel <- train(T.obs ~  . , data = trainingData, method = "rf", trControl = controlObject) 
    ...
    enetModel <- train(T.obs ~ . , data = trainingData , method = "enet", preProc = c("center", "scale"), tuneGrid = enetGrid, trControl = controlObject)
    ...
    set.seed(669); ptm <- proc.time()
    plsModel <- train(T.obs ~ . , data = trainingData , method = "pls", preProc = c("center", "scale"), tuneLength = 15, trControl = controlObject)
    ...
    # set.seed(669); ptm <- proc.time()
    # svmRModel <- train(T.obs ~ . , data = trainingData, method = "svmRadial",
    ...
    ######################################################## Bagged Tree
    set.seed(669); ptm <- proc.time()
    treebagModel <- train(T.obs ~ . , data = trainingData, method = "treebag", trControl = controlObject)
    ...
    ####################################################### Cond Inf Tree
    set.seed(669); ptm <- proc.time()
    ctreeModel <- train(T.obs ~ . , data = trainingData , method = "ctree", tuneLength = 10, trControl = controlObject)
    ...
    ######################################################## CART
    set.seed(669); ptm <- proc.time()
    rpartModel <- train(T.obs ~ . , data = trainingData , method = "rpart", tuneLength = 30, trControl = controlObject)
    ...
    prediction <- cbind( rownames(testData), testData[,30], pred.lm, pred.enet,  pred.pls, pred.tree, pred.ctree, pred.rpart, loc[i], 'temperature obs')
    ...
    saveRDS(list.model[j],  
      paste("G:/My Drive/01. BMKG/MOS/data/model/T_",model[j],"_",loc[i],".rds",sep=""))
    }
...
write.csv(eval, "G:/My Drive/01. BMKG/MOS/data/Temperature obs model assessment.csv", row.names=F)
write.csv(pred, "G:/My Drive/01. BMKG/MOS/data/Temperature obs model prediction.csv", row.names=F)
write.csv(best_fit_model, "G:/My Drive/01. BMKG/MOS/data/Temperature best fit model.csv", row.names=F)
